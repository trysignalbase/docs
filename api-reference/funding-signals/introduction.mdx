...
| `investor_name` | string | Search by investor name (partial match, substring — `Sequoia` also matches `Sequoia Capital China`) |
| `investors` | string | Comma-separated investor names matched **exactly** (case- and whitespace-insensitive) with OR semantics — rounds where any listed investor participated (e.g. `a16z,Sequoia Capital`). Use this when `investor_name` is too loose; if both are supplied they are ANDed |
...
- **200**: Success
- **400**: Bad request - an unknown query parameter name (the error lists the supported parameters and suggests near-misses) or an invalid filter value such as an unrecognized `round`
