...
| `email` | A specific person | Resolves **current employees only** today — job-change records carry no email. Returns an empty set when no employee holds that address. |
...
| `email` | string | Person email — resolves current employees only today |
