...
Each event type then adds one type-specific block. Monetary `amount` fields are **whole US dollars** (integers, not cents); `currency` is the stored symbol (e.g. `"$"`). See [Amounts & Currency](/enums#amounts--currency).
