...
Webhooks push real-time signal data to **your own HTTPS endpoint** the moment events happen — funding rounds, acquisitions, job changes, new hires, newly discovered companies, and newly discovered investors. Instead of polling, your system receives a signed `POST` with the full event payload.
...
| `investor.created` | A new investor (VC, angel, PE, …) is discovered. |
...
| --- | --- | --- | --- |
| `countries` | all signal types | Comma-separated ISO 3166-1 alpha-2 codes, e.g. `"US,GB"` | the company HQ country is in the list; for `hiring.created` events the job listing country also counts (job country OR HQ country, mirroring the REST hiring endpoint) |
| `jobCountries` | `hiring.created` only | Comma-separated ISO 3166-1 alpha-2 codes, e.g. `"US,GB"` | the job listing country is in the list (ignores company HQ — mirrors the REST `job_countries` param) |
...
Every **company signal** payload shares a common envelope inside `data`:
...
```json investor.created
{
  "data": {
    "investor": {
      "id": "f8a6b7c9-1d2e-4f3a-4b5c-6d7e8f9a0b1c",
      "name": "Nova Ventures",
      "type": "VC",
      "website": "https://nova.vc",
      "linkedinUrl": "https://www.linkedin.com/company/nova-ventures",
      "twitterUrl": null,
      "logoUrl": "https://logo.clearbit.com/nova.vc",
      "countries": ["US", "GB"],
  ...
  "event_id": "6e7f8091-a223-4b41-9c2d-4e5f6a7b8076",
...
In `investor.created` payloads, `type` is the original scraped value (free text, e.g. `VC`, `Corporate VC`, `Solo angel` — see [Investor Types](/enums#investor-types)), `countries` is an array of ISO 3166-1 alpha-2 codes, and `ticketSizeMin` / `ticketSizeMax` are whole USD amounts. Nullable fields are `null`, and `countries` may be empty.
...
- **Monitor webhooks are managed by their monitor.** A webhook created through a [monitor](/api-reference/monitors/introduction) shows up in List Webhooks and supports test, delivery listing, retry, and `rotate_secret` — but changing its URL, event types, filters, or `active` flag, or deleting it, returns `409 conflict`. Use [Update Monitor](/api-reference/monitors/endpoint/update) instead.
...
| 409 | `conflict` | Retrying a delivery that already succeeded, or editing/deleting a webhook that belongs to a [monitor](/api-reference/monitors/introduction#the-monitors-webhook). |
