...
Values for the [People API](/api-reference/people/introduction) (`GET /api/v2/people`). These use their own taxonomy — matched by word boundaries on job titles — and are distinct from the Job Change position/department enums above.
...
<Note>
The underlying `type` field is **free-text** (scraped values like `VC`, `VC firm`,
...
use the `search` parameter, which also matches the `type` field.

The `type` field **in the response is the original scraped value** (e.g. `Corporate VC`),
not the slug you filtered by — so a single `type=vc` query returns rows whose `type`
reads `VC`, `Venture Capital`, `Corporate VC`, and so on.
</Note>
