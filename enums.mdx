...
**There is no server-side currency conversion at query time.** The ingestion pipeline converts all amounts to USD before storage, so the vast majority of records have `currency` set to `"USD"` or `"$"`. Passing `currency` as a filter narrows results to only rows that were stored with that exact currency value.
