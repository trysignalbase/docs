...
Values accepted by the `round` filter, ordered by prevalence. Values are stored
...
<Note>
The `round` filter is an **exact, case-insensitive match** — `Series A`,
...
rows matched by your other filters.
</Note>
...
<Warning>
Amounts are **not converted to a common currency** — each `amount` is expressed in its own `currency` (most are `$`). If your results span multiple currencies, an `amount_min` / `amount_max` filter compares the raw numbers regardless of currency, so add a `currency` filter (e.g. `currency=$`) when you need an apples-to-apples range.
</Warning>

#### Currency Values

Currency is recorded differently on the two endpoints, so filter accordingly:

- **Funding** uses **ISO 4217 codes**. Most records are `USD`; the rest are mostly `EUR` and `GBP`. Filter with the exact code, e.g. `?currency=USD`. Common values:

  ```json copy
  ["USD", "EUR", "GBP", "CHF", "JPY", "INR", "CAD", "AUD", "SEK", "CNY", "NZD", "KRW", "SGD", "DKK", "BRL"]
  ```

- **Acquisitions** report the currency as **`$`** for most records, with a small number using `USD` or `EUR`. Filter with `?currency=$`.
